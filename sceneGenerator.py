...
def save_values(name,address,metal,rough,colorR,colorG,colorB):
    f = open(address + name + ".txt", "x")
    textt = "{met:.3f} {roug:.3f} {cR:.3f} {cG:.3f} {cB:.3f}"
    f.write(textt.format(met = metal, roug = rough, cR = colorR, cG = colorG, cB = colorB))
    f.close()
...
def generate_scene(name,address):
    ...
    colorR = random.uniform(0.01, 0.9)
    colorG = random.uniform(0.01, 0.9)
    colorB = random.uniform(0.01, 0.9)
